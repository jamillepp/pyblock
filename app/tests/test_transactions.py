...
def test_validate_eth_transaction(client):
    ...
    response = client.post("/transactions/validate", params={"tx_hash": ETH_TX_HASH})
    assert response.status_code == 200
    data = response.json()
    ...
    assert data["transaction_type"] == "eth"

def test_validate_erc20_transaction(client):
    """Test validating an ERC20 transaction."""

    response = client.post("/transactions/validate", params={"tx_hash": ERC20_TX_HASH})
    assert response.status_code == 200
    data = response.json()
    ...
    assert "receipt_status" in data
    assert "transaction_type" in data
    assert data["transaction_type"] == "erc20"
